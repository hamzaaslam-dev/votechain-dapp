...
function handleApply(req, res) {
  ...
  if (store.hasPendingDuplicate(cnic, blindedToken)) {
  ...
  const entry = store.addApplication({ fullName, phone, cnic, blindedToken });
...
async function handleAdminMarkApproved(req, res) {
  ...
  store.setApproved(id, signedToken);
  return res.json({ ok: true });
}
...
function handleGetStatus(req, res) {
  ...
  const app = store.getByCnic(cnic);
  if (!app) {
