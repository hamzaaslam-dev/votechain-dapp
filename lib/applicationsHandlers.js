...
async function handleAdminMarkApproved(req, res) {
  ...
  const { adminD, adminN } = await getAdminKey();
  const signedToken = BlindSignature.sign(BigInt(row.blindedToken), adminD, adminN).toString();
...
async function handleGetStatus(req, res) {
  ...
  return res.json({ ok: true, status: app.status, signedToken: app.signedToken });
}
