...
async function handleApply(req, res) {
  ...
  const blindedToken = String(body.blindedToken || "").trim();
  ...
  if (!blindedToken) {
    return res.status(400).json({ ok: false, message: "Invalid blinded token" });
  }

  if (await store.hasPendingDuplicate(cnic, blindedToken)) {
    return res.status(409).json({
      ok: false,
      message: "Duplicate pending application (same CNIC or same token)"
    });
  }

  const entry = await store.addApplication({ fullName, phone, cnic, blindedToken });
...
async function handleAdminMarkApproved(req, res) {
  ...
  const { adminD, adminN, adminE } = await getAdminKey();
  const blinded = BigInt(row.blindedToken);
  const signedToken = BlindSignature.sign(blinded, adminD, adminN).toString();

  // Sanity check: admin signature must match this deployment's key pair
  if (!BlindSignature.checkBlindedSignature(signedToken, blinded, adminE, adminN)) {
    return res.status(500).json({
      ok: false,
      message: "Admin key mismatch — re-apply after fixing KV / ADMIN_RSA_KEY_JSON"
    });
  }

  await store.setApproved(id, signedToken);
  return res.json({ ok: true });
}
...
async function handleGetStatus(req, res) {
  ...
  return res.json({
    ok: true,
    status: app.status,
    signedToken: app.signedToken != null ? String(app.signedToken) : null
  });
}
