...
function handleApply(req, res) {
  ...
  if (!isBytes32Hex(commitment)) {
    return res.status(400).json({ ok: false, message: "Invalid commitment (0x + 64 hex chars)" });
  }

  if (store.hasPendingDuplicate(cnic, commitment)) {
    return res.status(409).json({
      ok: false,
      message: "Duplicate pending application (same CNIC or same commitment)"
    });
  }
