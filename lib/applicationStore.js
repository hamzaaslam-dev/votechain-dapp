...
async function addApplication(fields) {
  ...
  const entry = {
    ...
    votingTokenHash: String(fields.votingTokenHash || "").trim()
  };
...
async function hasPendingDuplicate(cnic, votingTokenHash) {
  try {
    const apps = (await kv.hgetall("applications")) || {};
    return Object.values(apps).some(
      (a) =>
        a.status === "pending" &&
        (a.cnic === cnic || a.votingTokenHash === votingTokenHash)
    );
  } catch (e) {
...
async function setVoted(cnic) {
  const row = await getByCnic(cnic);
  if (!row) return null;

  row.votedAt = new Date().toISOString();

  try {
    await kv.hset("applications", { [row.id]: row });
  } catch (e) {
    console.warn("[applicationStore] KV error on setVoted:", e.message);
  }
  return row;
}
