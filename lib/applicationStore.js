...
function hasPendingDuplicate(cnic, blindedToken) {
  return readAll().some(
    (a) =>
      a.status === "pending" &&
...
function getByCnic(cnic) {
  return readAll().find((a) => a.cnic === cnic) || null;
}

function setApproved(id, signedToken) {
  const list = readAll();
  const i = list.findIndex((a) => a.id === id);
  if (i === -1) return null;
  list[i].status = "approved";
  list[i].approvedAt = new Date().toISOString();
  list[i].signedToken = signedToken || null;
  applications = list;
  persist();
  return list[i];
}
