...
async function verifyBallotAdminSession(ballotAddress, message, signatureBase64, walletBase58) {
  ...
  let onChainAdmin;
  try {
    onChainAdmin = await readBallotAdmin(connection, ballotPk.toBase58());
  } catch {
    return { ok: false, error: "Could not read ballot on Solana (check SOLANA_RPC_URL)" };
  }
  if (!onChainAdmin) {
    return { ok: false, error: "Ballot account not found" };
  }
  if (!onChainAdmin.equals(walletPk)) {
